...
export default function EditPetScreen({ route, navigation }: any) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setBreed(data.breed);
        setGender(data.gender ?? 'Samiec');
        setWeight(data.weightKg?.toString() ?? '');
        setFurColor(data.furColor ?? FUR_COLORS[0]);
        if (data.dateOfBirth) {
        ...
        Alert.alert('Błąd', 'Nie udało się załadować danych pupila');
      } finally {
  ...
  const handleSave = async () => {
    if (!name.trim()) {
      Alert.alert('Błąd', 'Podaj imię zwierzaka'); return;
    }
    if (!breed.trim()) {
      Alert.alert('Błąd', 'Podaj rasę'); return;
    }
    if (!dateOfBirth.trim()) {
      Alert.alert('Błąd', 'Podaj datę urodzenia'); return;
    }
    ...
    try {
      ...
      Alert.alert('Sukces', 'Dane pupila zostały zaktualizowane!', [
        { text: 'OK', onPress: () => navigation.goBack() },
      ]);
    } catch (e: any) {
      Alert.alert('Błąd', e.response?.data?.message ?? 'Nie udało się zaktualizować danych');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Edytuj pupila</Text>
        <TouchableOpacity onPress={() => navigation.goBack()}>
